...
import {
  ...
  esc, clean, fmtStr, num, linear, piecewise, text, textLines, line, rect, hit, hitCircle, mark, dot, legend, wrap, textWidth,
  table, breakMark, parseDate, tipFields, R,
} from './lib.mjs';
...
function swimlanes(o) {
  const { lanes, scale, ticks, labelW, left, right, breaks = [], bands = [], legendItems, headH = 0 } = o;
  const parts = [];
  let y = 0;
  ...
  for (const ln of lanes) {
    const items = ln.items.map((it) => ({ ...it, a: scale(it.x0), b: it.x1 !== undefined ? scale(it.x1) : scale(it.x0) })).sort((p, q) => p.a - q.a);
    const ends = [];
  ...
  for (const b of bands) {
    const x0 = scale(b.x0);
    const x1 = scale(b.x1);
    parts.push(rect(x0, top - (b.labelUp ? 24 : 0), x1 - x0, bottom - top + (b.labelUp ? 24 : 0), BAND));
    if (b.label) parts.push(text(b.labelAnchor === 'end' ? x0 - 4 : x1 + 4, top - 12, b.label, { size: 12, fill: INK, anchor: b.labelAnchor || 'start' }));
  }
  ...
  for (const L of laneLayout) {
    ...
    parts.push(textLines(labelW, L.y + L.h / 2 - ((L.ll.length - 1) * 15) / 2, L.ll, { size: 12.5, fill: INK, anchor: 'end', lineH: 15 }));
    let lastEnd = left;
...
export function chartTimeline(spec, D) {
  ...
  const extra = ms.filter((r) => r.jurisdiction === 'Vietnam' && r.status === 'in force' && !tlDates.has(r.date) && (parseDate(r.date) ?? 9999) <= NOW);
  const RULES = 'Rules in force (Vietnam)';
  const pref = ['plant-based meat', 'plant-based dairy', 'traditional chay industrial', 'mushroom/mycelium', 'microalgae', 'feed SCP/yeast', 'insect (benchmark)', 'cultivated', 'ingredient supplier', 'other'];
  const cats = uniq([...pref.filter((c) => tl.some((r) => r.category === c)), ...tl.map((r) => r.category)]);
  const left = 176;
  const right = W - 14;
  ...
  const lanes = cats.map((c) => ({
    ...
    items: tl
      ...
      .map((r) => ({ x0: parseDate(r.date), color: S(1), tip: [`${r.date}: ${r.event}`, ...tipFields(r, spec.encoding.tooltip)] })),
  }));
  lanes.push({
    label: RULES,
    items: extra.map((r) => ({ x0: parseDate(r.date), color: S(3), tip: [`${r.date}: ${r.event}`, `Instrument: ${r.instrument}`, `Relevance: ${r.relevance}`] })),
  });
  const ticks = [
    { v: 1976, label: '1976' },
    { v: 2000, label: '2000' },
  ...
  const note = text(0, G.height, 'Before 2019 the axis is compressed. Each dot is one event; hover or open the data table for details.', { size: 12 });
  const tRows = [
    ...tl.map((r) => ({ date: r.date, lane: cap(r.category), event: r.event, actors: r.actors, significance: r.significance, x: parseDate(r.date) })),
    ...extra.map((r) => ({ date: r.date, lane: RULES, event: r.event, actors: r.instrument, significance: r.relevance, x: parseDate(r.date) })),
  ].sort((a, b) => a.x - b.x);
  ...
  return { body: G.body + note, height: G.height + 14, table: tbl, excluded: ms.filter((r) => r.jurisdiction === 'Vietnam' && r.status === 'in force' && (parseDate(r.date) ?? 0) > NOW).map((r) => `${r.record_id} (${r.date})`) };
}
...
export function milestonesTimeline(spec, D) {
  ...
  const statusOrder = ['in force', 'adopted', 'planned', 'target', 'projected', 'observed', 'published'];
  const stOf = (r) => (statusOrder.includes(norm(r.status)) ? norm(r.status) : 'other');
  const present = [...statusOrder.filter((s) => rows.some((r) => stOf(r) === s)), ...(rows.some((r) => stOf(r) === 'other') ? ['other'] : [])];
  ...
  const lanes = jur.map((j) => ({
    ...
    items: rows
      ...
      .map((r) => {
        ...
        return { x0: a, x1: b, color: color(stOf(r)), tip: [`${r.date}: ${r.event}`, `Status: ${r.status}`, ...tipFields(r, spec.encoding.tooltip)] };
      }),
  }));
  const ticks = [2025, 2026, 2027, 2028, 2030, 2032, 2034].map((v) => ({ v, label: String(v) })).concat([{ v: 2049, label: '2049' }]);
  // annotation: shade October to November 2026 (Food Safety Law session)
  const G = swimlanes({
    legendItems: present.map((s) => ({ label: s === 'other' ? `Other (${otherVals.join(', ')})` : cap(s), color: color(s), shape: 'dot' })),
    headH: 26,
    lanes,
    scale,
    ticks,
    ...
    right,
    breaks: [right - 52],
    bands: [{ x0: 2026.75, x1: 2026 + 11 / 12, label: 'Food Safety Law session, October to November 2026', labelAnchor: 'start', labelUp: true }],
  });
  const tbl = table(spec.title, [
    { key: 'date', label: 'Date' },
  ...
  const note = text(0, G.height, 'The axis is compressed after 2028 and broken between 2036 and 2049. Ranges are drawn as bars.', { size: 12 });
...
export function regionalApprovals(spec, D) {
  ...
  const catOf = (c) => (c === 'cultivated' ? 'Cultivated' : c === 'biomass' ? 'Biomass' : c === 'PF' ? 'Precision fermentation (PF)' : 'Other');
  const cats = ['Cultivated', 'Biomass', 'Precision fermentation (PF)', 'Other'];
  const color = (c) => S(cats.indexOf(catOf(c)) + 1);
  const otherVals = uniq(rows.filter((r) => catOf(r.category) === 'Other' && r.category !== 'n/a').map((r) => r.category.replace(/^other \((.*)\)$/, '$1')));
  const notApproval = (r) => /application only|not a food|no approval|no novel food/i.test(r.form_of_approval);
  const labelW = 150;
  const left = labelW + 16;
  const right = W - 14;
  ...
  const lanes = countries.map((c) => {
    ...
    for (const r of rs) {
      ...
      const tip = [`${r.country}, ${r.date}: ${r.product}`, `Category: ${r.category}`, `Form: ${r.form_of_approval}`, ...tipFields(r, spec.encoding.tooltip)];
  ...
  const G = swimlanes({
    legendItems: [
      ...cats.map((c) => ({ label: c === 'Other' ? `Other (${otherVals.join(', ')})` : c, color: S(cats.indexOf(c) + 1), shape: 'dot' })),
      { label: 'Application or ruling, not an approval', color: MUTED, shape: 'hollow' },
    ],
    lanes,
    scale,
    ticks: [2019, 2020, 2021, 2022, 2023, 2024, 2025, 2026, 2027].map((v) => ({ v, label: String(v) })),
    labelW,
    left,
    right,
  });
...
function clamp(lines, n, maxW, size, weight) {
  ...
  let last = out[n - 1];
  while (last.includes(' ') && textWidth(last + '…', size, weight) > maxW * 0.97) last = last.replace(/\s+\S+$/, '');
...
export function visionBackcast(spec, D) {
  ...
  const pillars = ann ? ann.replace(/^Lanes in this order:\s*/, '').replace(/\.$/, '').split(/,\s*/) : uniq(rows.map((r) => r.pillar));
  const labelW = 84;
  ...
  parts.push(`<rect x="1" y="1" width="${R(bw)}" height="22" rx="11" style="fill:none;stroke:${S(5)};stroke-width:1.5"/>`);
  parts.push(text(11, 12, badge, { size: 12, fill: INK, weight: 600 }));
  let y = 36;
  periods.forEach((p, j) => parts.push(text(left + j * cw + 6, y + 8, p, { size: 12.5, fill: INK, weight: 600 })));
  y += 20;
  ...
  for (const pl of pillars) {
    ...
    cells.forEach((c, j) => {
      ...
      const x = left + j * cw + 6;
      for (const it of c) {
        const tip = [`${it.r.period}, ${it.r.pillar}: ${it.r.milestone}`, `${it.r.indicator}: ${it.r.value_or_range}`, ...tipFields(it.r, spec.encoding.tooltip)];
        const inner =
          dot(x + 4, cy + 1, 4, S(5)) +
          textLines(x + 14, cy + 1, it.v, { size: 12, fill: INK, weight: 600, lineH: 14.5 }) +
          textLines(x + 14, cy + 1 + it.v.length * 14.5, it.ind, { size: 12, fill: MUTED, lineH: 14 }) +
  ...
  for (let j = 1; j < periods.length; j++) parts.push(line(left + j * cw, colTop, left + j * cw, y, GRID, 1));
  const tbl = table(spec.title, [
