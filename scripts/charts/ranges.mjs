...
import {
  ...
  fmtStr, fmtNum, num, linear, logScale, text, textLines, line, rect, hbar, rangeBar, vRangeBar, hit, hitCircle, mark, dot,
  tipFields, legend, wrap, textWidth, yAxis, table, breakMark, circle,
} from './lib.mjs';
...
export function feedstockVolumes(spec, D) {
  ...
  const L = hLayout({
    ...
    drawRow: (r, i, ctx, y, h) => {
      ...
      const bh = 16;
      const by = y + (h - bh) / 2;
      const tip = [`${r.stream}: ${rangeTxt(r.volume_mt_low, r.volume_mt_high)} Mt`, `Protein: ${r.protein_note}`, ...tipFields(r, spec.encoding.tooltip)];
      const lab = `${rangeTxt(r.volume_mt_low, r.volume_mt_high)} Mt`;
      if (isRef(r)) {
        const x1 = ctx.scale(hi);
        const shape = hbar(ctx.plotLeft + 0.75, x1, by + 0.75, bh - 1.5, 'none', `;stroke:${INK};stroke-width:1.5`);
        return mark(tip, shape + hit(ctx.plotLeft, y, x1 - ctx.plotLeft + 4, h)) + endLabel(x1, y + h / 2, lab, W, { size: 12, weight: 600 });
      }
      if (hi > DMAX) {
        // clipped bar with a break marker (annotation: clip paddy with a break marker)
        const xe = ctx.scale(DMAX) + 6;
        const shape = hbar(ctx.plotLeft, xe, by, bh, color(r)) + breakMark(ctx.scale(DMAX) - 12, y + h / 2, bh + 4);
        return mark(tip, shape + hit(ctx.plotLeft, y, xe - ctx.plotLeft, h)) + text(xe + 6, y + h / 2, lab, { size: 12, fill: INK });
      }
      const shape = solidPlusRange(ctx.scale, ctx.plotLeft, lo, hi, by, bh, color(r));
      const x1 = Math.max(ctx.scale(hi), ctx.plotLeft + 2);
      return mark(tip, shape + hit(ctx.plotLeft, y, x1 - ctx.plotLeft + 4, h)) + endLabel(x1, y + h / 2, lab, W, { size: 12 });
    },
    under: (ctx) => {
      const x = ctx.scale(refV);
      return line(x, ctx.top - 8, x, ctx.bottom, INK, 1) + text(x, ctx.top - 16, `Soybean meal fed, ${fmtStr(refRow.volume_mt_high)} Mt`, { size: 12, fill: INK, anchor: 'middle' });
    },
  });
...
export function priceToBeat(spec, D) {
  ...
  const L = hLayout({
    ...
    drawRow: (r, i, ctx, y, h) => {
      ...
      const tip = [`${r.item}: USD ${rangeTxt(r.usd_per_t_protein_low, r.usd_per_t_protein_high)} per t of protein`, ...tipFields(r, spec.encoding.tooltip)];
      const lab = `USD ${rangeTxt(r.usd_per_t_protein_low, r.usd_per_t_protein_high)}`;
      const lw = textWidth(lab, 12);
      const labSvg = x1 + 6 + lw <= W ? text(x1 + 6, y + h / 2, lab, { size: 12, fill: INK }) : text(x0 - 6, y + h / 2, lab, { size: 12, fill: INK, anchor: 'end' });
      return mark(tip, rangeBar(x0, x1, y + (h - bh) / 2, bh, color(r.kind)) + hit(x0 - 4, y, x1 - x0 + 8, h)) + labSvg;
    },
  });
  const tbl = table(spec.title, [
    { key: 'item', label: 'Item' },
...
export function capexPerTonne(spec, D) {
  ...
  const L = hLayout({
    ...
    drawRow: (r, i, ctx, y, h) => {
      ...
      const bh = 20;
      const tip = [`${r.case}: USD ${rangeTxt(r.usd_per_annual_t_low, r.usd_per_annual_t_high)} per annual t`, ...tipFields(r, spec.encoding.tooltip)];
      const x1 = ctx.scale(hi);
      return (
        mark(tip, solidPlusRange(ctx.scale, ctx.plotLeft, lo, hi, y + (h - bh) / 2, bh, S(1)) + hit(ctx.plotLeft, y, x1 - ctx.plotLeft + 4, h)) +
        endLabel(x1, y + h / 2, `USD ${rangeTxt(r.usd_per_annual_t_low, r.usd_per_annual_t_high)}`, W, { size: 12 })
      );
    },
  });
  const tbl = table(spec.title, [
...
export function costPaths2050(spec, D) {
  ...
  parts.push(yAxis(y, [0, 2, 4, 6, 8, 10, 12], left, right, fmtNum, { title: 'USD per kg of protein', titleX: left - 42, titleY: top - 26 }));
  ...
  parts.push(mark([`${fishmeal.series.replace(/^Reference: /, '')}: USD ${rangeTxt(fishmeal.usd_per_kg_protein_low, fishmeal.usd_per_kg_protein_high)} per kg`, ...tipFields(fishmeal, spec.encoding.tooltip)], rect(left, fy1, right - left, fy0 - fy1, BAND)));
  parts.push(text(right + 8, (fy0 + fy1) / 2, `Fishmeal ${rangeTxt(fishmeal.usd_per_kg_protein_low, fishmeal.usd_per_kg_protein_high)}`, { size: 12, fill: INK }));
  const sy = y(num(sbm.usd_per_kg_protein_low));
  parts.push(mark([`${sbm.series.replace(/^Reference: /, '')}: USD ${fmtStr(sbm.usd_per_kg_protein_low)} per kg`, ...tipFields(sbm, spec.encoding.tooltip)], line(left, sy, right, sy, INK, 1.5) + hit(left, sy - 5, right - left, 10)));
  parts.push(text(right + 8, sy, `Soybean meal ${fmtStr(sbm.usd_per_kg_protein_low)}`, { size: 12, fill: INK }));
  const ly = y(num(floor.usd_per_kg_protein_low));
  parts.push(mark([`${floor.series.replace(/^Reference: /, '')}: USD ${fmtStr(floor.usd_per_kg_protein_low)} per kg`, ...tipFields(floor, spec.encoding.tooltip)], line(left, ly, right, ly, INK, 1.5, ';stroke-dasharray:2 3;stroke-linecap:round') + hit(left, ly - 5, right - left, 10)));
  parts.push(text(right + 8, ly, `Fungal floor ${fmtStr(floor.usd_per_kg_protein_low)}`, { size: 12, fill: INK }));
  // bars
  const bw = 26;
  ...
  for (const r of data) {
    ...
    const tip = [`${r.series}, ${r.year}: USD ${rangeTxt(r.usd_per_kg_protein_low, r.usd_per_kg_protein_high)} per kg of protein`, ...tipFields(r, spec.encoding.tooltip)];
    parts.push(mark(tip, vRangeBar(cx - bw / 2, bw, y(lo), y(hi), S(si + 1)) + hit(cx - bw / 2 - 1, y(hi) - 16, bw + 2, y(lo) - y(hi) + 32)));
    parts.push(text(cx, y(hi) - 9, fmtStr(r.usd_per_kg_protein_high), { size: 12, fill: INK, anchor: 'middle', halo: true }));
    parts.push(text(cx, y(lo) + 10, fmtStr(r.usd_per_kg_protein_low), { size: 12, fill: INK, anchor: 'middle', halo: true }));
  }
  parts.push(line(left, base, right, base, AXIS, 1));
  for (const yr of years) parts.push(text(x(yr), base + 16, String(yr), { size: 13, fill: INK, anchor: 'middle' }));
  // annotation label
...
export function retailArchetypes(spec, D) {
  ...
  const L = hLayout({
    ...
    drawRow: (r, i, ctx, y, h) => {
      ...
      return mark(tip, whisk + dot(xm, cy, rad, S(1)) + hit(Math.min(xa, xm - 12), y, Math.max(xb - xa, 24) + 12, h)) + text(Math.max(xb, xm + rad) + 8, cy, `${fmtStr(r.median)} g`, { size: 12, fill: INK, halo: true });
    },
  });
  const tbl = table(spec.title, [
...
export function playsHorizon(spec, D) {
  ...
  const rows = D.csv(spec.data.file)
    ...
    .sort((a, b) => num(a.horizon_start_years) - num(b.horizon_start_years) || num(a.horizon_end_years) - num(b.horizon_end_years) || idn(a.play_id) - idn(b.play_id));
  const segs = uniq(D.csv(spec.data.file).filter((r) => r.type === 'play').map((r) => r.segment));
  ...
  const L = hLayout({
    ...
    drawRow: (r, i, ctx, y, h) => {
      ...
      if (more) shape += hbar(x1 - 4, ctx.scale(11), by, bh, color(r.segment), ';fill-opacity:0.35');
      const tip = [`${r.play_id} ${r.name}: ${r.horizon}`, `Segment: ${r.segment}`, ...tipFields(r, spec.encoding.tooltip)];
      const lab = r.horizon;
      const lw = textWidth(lab, 12);
      const xe = more ? ctx.scale(11) : x1;
      const labSvg = xe + 6 + lw <= W ? text(xe + 6, y + h / 2, lab, { size: 12, fill: INK }) : text(x0 - 6, y + h / 2, lab, { size: 12, fill: INK, anchor: 'end' });
...
export function frontierWindows(spec, D) {
  ...
  const L = hLayout({
    legend: [...main.map((f) => ({ label: f, color: color(f) })), ...(other.length ? [{ label: `Other (${other.join(', ').toLowerCase()})`, color: S(8) }] : [])],
    rows: rows.map((r) => ({ data: r, label: r.route, sub: `${r.family}; ${r.window_start === r.window_end ? `not before ${r.window_end}` : `${r.window_start} to ${r.window_end}`}` })),
    labelW: 270,
    plotRight: W - 16,
    minRowH: 34,
    ...
    drawRow: (r, i, ctx, y, h) => {
      ...
      const bh = 16;
      const by = y + (h - bh) / 2;
      const tip = [`${r.route}: ${a === b ? `not before ${b}` : `${a} to ${b}`}`, `Family: ${r.family}`, ...tipFields(r, spec.encoding.tooltip)];
      if (a === b) {
        // annotation: draw the cell-free row as a single dot at 2050 with the label "not before 2050"
        const cx = ctx.scale(b) - 7;
        return mark(tip, dot(cx, y + h / 2, 6, color(r.family)) + hitCircle(cx, y + h / 2, 12)) + text(cx - 14, y + h / 2, 'not before 2050', { size: 12, fill: INK, anchor: 'end' });
      }
      const x0 = ctx.scale(a);
      const x1 = ctx.scale(b);
