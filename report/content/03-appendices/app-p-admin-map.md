---
id: app-p-admin-map
title: "P. Administrative map after the 2025 reforms"
short_title: "P. Admin map"
section: appendix
order: 16
...
related_pages: [ch02-asset-map, ch03-industrial-base, app-c-feedstocks, app-f-facilities, app-g-regulation, app-o-outlook, app-e-directory]
charts: []
---

# P. Administrative map after the 2025 reforms
...
## P.2 The 34 provincial-level units
...
## P.3 Seven units named after one former province and run from another
...
## P.4 Where the alternative-protein clusters now sit

The clusters below are the ones this report relies on. Each row gives the current unit, the former unit in brackets, and the evidence that places the activity there. Details are in [[app-c-feedstocks]] and [[app-f-facilities]].
...
## P.5 The end of the district tier
...
## P.6 Ministries after the 2025 restructuring
...
## P.7 Practical rules for reading pre-2025 statistics and addresses
...
3. **Watch the split-name units.** In the seven units in section P.3, the provincial offices sit in a different former province from the one that gave the unit its name.
...
8. **Mind the population sources.** Resolution 202 gives dossier figures with no stated reference date; the statistics office and the UN give national totals that differ by about 0.7 million for 2025 (see [[app-o-outlook]]) [@GT-01; @MAC-19; @GT-12] {VN-direct|Medium}.
...
- `provinces.csv`: the 34 units with type, administrative centre, former units, area and population (section P.2).
- `ministries.csv`: the six ministry and provincial rows (section P.6).
- `facilities.csv`, `feedstocks.csv` and `companies.csv`: carry `province_current` and `province_former` columns mapped with `provinces.csv` (see [[app-f-facilities]], [[app-c-feedstocks]] and [[app-e-directory]]).
- `disagreements.csv`: DG-123 and DG-124 (see [[app-r-disagreements]]).
