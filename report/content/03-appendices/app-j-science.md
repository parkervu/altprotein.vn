---
id: app-j-science
title: "J. State of the science by technology family"
short_title: "J. Science"
section: appendix
order: 10
...
related_data: [science_facts.csv, tea_benchmarks.csv, replacement_trials.csv, publications.csv, biblio_counts.csv, cost_inputs.csv, disagreements.csv]
related_pages: [ch10-technology-fit, ch09-economics, app-i-costs, app-k-research, app-l-labs-talent, app-h-feed-market, app-g-regulation, app-r-disagreements, brief-research]
charts: [chart-technology-fit-matrix]
---

# J. State of the science by technology family

This appendix sets out the state of the science in September 2026 for each technology family that could supply alternative protein from Vietnam. For each family it gives the key numbers with their DOIs, what the evidence means for Vietnam, and where our earlier drafts were wrong. [[ch10-technology-fit]] draws the conclusions; [[app-i-costs]] holds the full cost inputs; [[app-k-research]] holds the Vietnamese research record.
...
|---|---|---|---|---|
| Plant proteins and texturisation | Mature globally; no verified capex benchmark for extrusion found | Soy isolate DIAAS 97 (pigs) | About 5 plant-based meat papers; no Vietnamese extrusion study (edition 1.1) | [@SCI-40; @HSC-01] {general\|High} |
...
- **Protein quality of common isolates and concentrates** is covered in section J.3. Soy isolate is the reference: measured pig DIAAS of 97 on the older child and adult pattern [@SCI-40] {general|High}.
...
> **Correction.** Wave 1 counted only 2 Vietnamese papers on plant-based meat and none on extrusion or texturisation [@RD-01] {VN-direct|Medium}. The wave 2 bibliometric review, using a second database, found about 6 on-topic plant-based meat papers from 2015 to 2026 and at most one extrusion study [@BIB-01] {VN-direct|Medium}. See DG-138 and DG-139. Edition 1.1: the one extrusion candidate is Taiwanese, so Vietnam has none and about 5 plant-based meat papers [@HSC-01] {VN-direct|Medium}.
...
| Ingredient | DIAAS, older child and adult pattern | DIAAS, young child pattern | Limiting amino acid | Source (DOI) | Evidence |
|---|---|---|---|---|---|
| Soy protein isolate | 97 | 83 | Sulphur amino acids | doi:10.1002/jsfa.12809 | [@SCI-40] {general\|High} |
...
- **Blends.** We found no measured (in vivo) rice plus legume blend with a DIAAS of 100 or more. Claims of 100 come from calculations, for example pea plus rice blends reaching a PDCAAS of 1.00 on the adult pattern (doi:10.1139/apnm-2021-0806) [@SCI-43] {general|Medium}.
- **Microalgae and duckweed.** No DIAAS for microalgal biomass was available as of a 2021 review (doi:10.3390/foods10123002) [@SCI-39] {general|Medium}. For duckweed we found only amino-acid scores and in vitro digestibility (section J.8), and no PDCAAS or DIAAS study [@SCI-34; @SCI-38] {general|Low}.

### Vietnamese evidence

- We found no Vietnamese DIAAS or PDCAAS study [@RD-01] {VN-direct|Medium}. In vitro digestibility has been reported, for example 77.1% for rice DDG protein [@RD-23] {VN-direct|High}.
...
- For legal routes (livestock list, aquafeed trials, penalties since August 2026), see [[app-g-regulation]].
...
- *K. marxianus* links precision fermentation to thermotolerance (section J.13), which suits Vietnam's climate [@SCI-04; @SCI-44] {general|High}.
- Legally, a precision-fermented protein is probably a "product of a GMO" in Vietnam, and a plant using a GM strain needs MAE's closed-production certificate [@REG2-11] {VN-direct|Medium}. See [[app-g-regulation]].
...
- **Approvals.** Singapore lists six cultivated products, including the world's first cultivated duck (April 2026) and beef (July 2026) [@RGN-04] {VN-adjacent|High}. The US FDA has completed five cultivated-cell consultations, including Wildtype's coho salmon on 28 May 2025, the first cultivated seafood cleared in the US [@RGN-03] {general|High}. See [[app-n-regional]].
...
This table collects every science claim from earlier drafts that the wave 2 review checked. Claims marked "Unsupported" or "Wrong" are not used anywhere in the report. Conflicts are logged in [[app-r-disagreements]].
...
| Vietnam has 2 plant-based meat papers and none on extrusion | Undercount | About 5 on-topic papers (2015 to 2026) and no extrusion study (edition 1.1) | [@BIB-01; @HSC-01] {VN-direct\|Medium} | DG-138, DG-139 |
...
- `tea_benchmarks.csv`: the 25 techno-economic benchmarks behind section J.16 (see [[app-i-costs]]).
- `replacement_trials.csv`: the 20 feed replacement trials behind sections J.5 and J.12 (see [[app-h-feed-market]]).
- `publications.csv`: Vietnam-affiliated publications, including those cited here (see [[app-k-research]]).
...
- `disagreements.csv`: DG-056, DG-080, DG-087, DG-088, DG-104 to DG-110 and DG-138 to DG-139 (see [[app-r-disagreements]]).
