# Wave 9: expansion lines (third wave)
...
## Output (write into working-papers/wave9/lines/<line-folder>/)

1. `<line>.md`, the working paper:
   - Title, then "Line: <ID and name> (wave 9). Source prefix: <PFX>. Date: 25 September 2026."
...
Waves 7 and 8 ran twelve lines; results are in working-papers/wave7/lines/ and working-papers/wave8/lines/ (wave 8: L1 label and product standards, L2 feed buyers, L3 export buyers, L4 nutrition quality, L5 e-commerce, L6 hybrid economics). Read the verdict, "What this changes" and "Next-wave candidates" sections of any line that touches yours; do not repeat its work; cite its sources by their IDs. Wave 9 is the stopping test: we expect lower yield. Be strict: if your line only adds citations, classify it as noise and stop early.

**Desktop browser (new, allowed for this wave).** The user has allowed use of the desktop browser pane where WebFetch fails or where a search is needed. Load its tools with ToolSearch (query "mcp__remote-devices__Claude_Browser__", max_results 64). Rules:
- Open your own tab with `mcp__remote-devices__Claude_Browser__preview_start` (it returns a tabId) and **always pass that tabId** to every browser call: other agents are using the same browser at the same time. Close your tab when done.
- Prefer `get_page_text` and `read_page` over screenshots. You may use site search boxes and a general search engine in the browser (for example https://www.google.com/search?q=... or https://www.bing.com/search?q=...) for this wave only.
- If a site asks for access, call `request_access` with scope "once" and wait; if it is declined, move on. Never log in, never create accounts, never solve CAPTCHAs, never submit forms other than search boxes, never download files you have not been asked to. Do not use social-media accounts the browser may be signed into; skip pages that need a login.
- If the browser tools are not available or fail repeatedly, fall back to WebFetch and record the gap.
